...
// createSlice() 함수: state 이름, 초기값 설정, 액션 및 리듀서 함수를 만들는 것을 도와줌
const counterSlice = createSlice({
  name: 'counter', // state 이름(추가로 action 타입을 만드는데도 쓰임)
  initialState, // state 초기값(위에서 객체 형태로 정의)
  reducers: { // reducer들을 정의
    increment: (state) => { // 첫번째 파라미터로: 현재 state를 받아옴(실제로는 state의 복사본)
      state.value += 1; // 배열 또는 객체의 경우에도 직접 수정하는 형태로 작성 가능!! 장점!
    ...
    incrementByAmount: (state, action) => { // 두번째 파라미터: actio 객체 { type: 발생한 액션의 타입 payload: 화물, 전달 데이터}
      console.log(action);
      console.log(state);
...
// 리덕스에서 사용하는 용어
// 1. 액션(Action)
// 전역 상태에 변화를 주고 싶을 때 액션이란 것을 발생 시킴
// 자바스크립트 객체 형태
// {
...
// 현재 state와 action 객체를 파라미터로 받아오고 필요한 경우 상태를 업데이틀하고 새 상태를 반환하는 함수
export default counterSlice.reducer; // 위에서 정리한 리듀서 함수들
