...
import axios from "axios"; // 중가로 생략하고 써야함
...
function ApiRequest() {
  ...
  const handleRequestById = (id) => {
    axios.get(`https://jsonplaceholder.typicode.com/photos/${id}`) // axios는 프로미스객체를 반환
      .then((response) => {
  ...
  // Quiz
  // 2. async/await로 리팩터링
  const handleRequestByIdAsyn = async (id) => {
    // axios.get(`https://jsonplaceholder.typicode.com/photos/${id}`) // axios는 프로미스객체를 반환
    //   .then((response) => {
    //     console.log(response);
    //     setData(response.data);
    ...
    //   });
    try {
      const respons = await axios.get(`https://jsonplaceholder.typicode.com/photos/${id}`);
  ...
  return (
    <>
      <div>
        ...
        <button type="button" onClick={() => handleRequestByIdAsyn(2)}>불러오기</button>
      </div>
      {data && (
        <>
